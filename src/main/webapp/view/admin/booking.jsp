...
<main style="margin-top: 58px;">
    <div class="container pt-4 ">
        <div class="row justify-content-center">
            <h2 class="col-12 text-center py-4 bg-body rounded shadow-sm text-info">Booking manager</h2>
            <!-- <div class="col-12 my-4">
              <a class="btn btn-success" href="/flight?aution=create" role="button">New Flight</a>
            </div> -->
            <div class="col-12 py-5 bg-body shadow-sm">
                    ...
                    <c:when test="${elementList.size()!=0}">
                        <table class="table table-bordered border-primary ">
                            ...
                            <tr>
                                ...
                                <th class="col" scope="col">Code ticket</th>
                                <th class="col" scope="col">Booking date time</th>
                            ...
                            <tbody>
                            <% int i=0;%>
                            <c:forEach items="${bookingList}" var="element">
                                <tr>
                                    <td scope="row"><%=i+=1%></td>
                                    <td>${element.getId()}</td>
                                    <td>${element.getBookingDate()}</td>
                                    <td>${element.getPrice()}</td>
                                    <td>${element.getState()}</td>
                                    <td class="d-flex justify-content-center">
                                        <a class="btn btn-info action-btn" href="/booking?action=adminViewDetails&id=${element.getId()}" role="button">View</a>
                                        <a class="btn btn-primary action-btn" href="/booking?action=adminEdit&id=${element.getId()}" role="button">Edit</a>
                                        <c:choose>
                                            <c:when test="${element.getState()=='NOT CHECK IN'}">
                                                <a class="btn btn-success" href="/booking?action=active&id=${element.getId()}&state=${element.getState()}" onclick="alertActive(${element.getId()},${element.getState()})" role="button">check in</a>
                                            </c:when>
                                            <c:otherwise>
                                                <a class="btn btn-danger wrapper-active-btn" href="/booking?action=active&id=${element.getId()}&state=${element.getState()}" onclick="alertActive(${element.getId()},${element.getState()})" role="button">check out</a>
                                            </c:otherwise>
                                        </c:choose>
                                    </td>
                                </tr>
                ...
                <div class="mt-5 d-flex justify-content-center">
                    <c:forEach begin="1" end="${indexPage}" var="index">
                        <a href="/booking?action=adminView&page=${index}" class="btn btn-link border">${index}</a>
                    </c:forEach>
                </div>
            </div>
...
<script>
    ...
    function alertActive(id,state){
        if (window.confirm('You Not Active Airplane Chair'))
        {
            fetch("/booking?action=active&id="+id+"&state="+state, {
